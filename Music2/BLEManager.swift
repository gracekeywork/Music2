...
class BLEManager: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    // These are the specific data channels inside the ESP32 that we talk through
    // One is for text/control, and the other two are for raw PCM audio data
    // These are the specific data channels inside the ESP32 that we talk through
    // One is for text/control, and one is for raw stereo PCM audio data
    ...
    // Control/text characteristic
    let controlCharacteristicUUID = CBUUID(string: "d4aff33c-2366-47c1-8460-3cc0b19517e6")

    // Single audio characteristic for stereo PCM
    let audioCharacteristicUUID = CBUUID(string: "6b23e947-8ecd-456c-a90b-e8888f4eca07")
    ...
    @Published var audioBackpressureCount = 0
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in peripheral.services ?? [] {
            ...
            // Ask for the characteristics inside this service
            peripheral.discoverCharacteristics(
                [controlCharacteristicUUID, audioCharacteristicUUID],
                for: service
            )
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in service.characteristics ?? [] {
            ...
            if characteristic.uuid == controlCharacteristicUUID {
                print("Matched control characteristic")
                controlCharacteristic = characteristic
                peripheral.setNotifyValue(true, for: characteristic)
            } else if characteristic.uuid == audioCharacteristicUUID {
                print("Matched audio characteristic")
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        // We only expect text notifications from the control characteristic
        guard characteristic.uuid == controlCharacteristicUUID else {
            print("Received update from non-control characteristic:", characteristic.uuid.uuidString)
            return
        }
    ...
    func sendCommand(_ command: String) {
        ...
        guard let peripheral = connectedPeripheral,
            ...
            print("sendCommand aborted - BLE control path not ready")
            return
        }
        ...
        print("Sending to ESP32 control characteristic:", command)
        print("Outgoing byte count:", data.count)

        // Write the bytes to the control characteristic on the ESP32
    ...
    // Sends one packet to audio characteristic A
    func sendAudioPacket(_ packet: Data) {
        guard let peripheral = connectedPeripheral else {
            print("sendAudioPacket aborted - no connected peripheral")
            return
        }

        guard let characteristic = audioCharacteristic else {
            print("sendAudioPacket aborted - no audio characteristic")
            return
        }

        let maxLength = peripheral.maximumWriteValueLength(for: .withoutResponse)

        guard packet.count <= maxLength else {
            print("sendAudioPacket aborted - packet too large: \(packet.count) bytes, max is \(maxLength)")
            return
        }

        if !peripheral.canSendWriteWithoutResponse {
            DispatchQueue.main.async {
                self.audioBackpressureCount += 1
            }

            print("Backpressure on audio - retrying shortly")
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
                self.sendAudioPacket(packet)
            }
            return
        }

        print("Sending stereo audio packet, bytes:", packet.count)
        peripheral.writeValue(packet, for: characteristic, type: .withoutResponse)
    }

    

    // Split raw Data into smaller BLE-sized chunks
    ...
    func testSendFakeStereoPCM() {
        let sampleRate = 8_000.0
        let frequency = 120.0
        let amplitude = 2400.0
        let durationSec = 10.0

        let frameCount = Int(sampleRate * durationSec)
        var raw = Data(capacity: frameCount * 4) // 4 bytes per stereo frame (Int16 L + Int16 R)

        for n in 0..<frameCount {
            let t = Double(n) / sampleRate
            let sampleValue = Int16((sin(2.0 * .pi * frequency * t) * amplitude).rounded())

            var left = sampleValue.littleEndian
            var right = sampleValue.littleEndian

            withUnsafeBytes(of: &left) { raw.append(contentsOf: $0) }
            withUnsafeBytes(of: &right) { raw.append(contentsOf: $0) }
        }

        // 160 bytes at 8 kHz stereo 16-bit = 5 ms of audio
        let packets = splitIntoChunks(raw, chunkSize: 160)

        for (index, packet) in packets.enumerated() {
            let delay = DispatchTime.now() + .milliseconds(index * 5)
            DispatchQueue.main.asyncAfter(deadline: delay) {
                print("Sending fake stereo sine packet \(index + 1)/\(packets.count)")
                self.sendAudioPacket(packet)
            }
        }
    }
    
    func testSendStereoPCMFile(from fileURL: URL) {
        do {
            let stereoData = try Data(contentsOf: fileURL)

            // 160 bytes at 8 kHz stereo 16-bit = 5 ms of audio
            let packets = splitIntoChunks(stereoData, chunkSize: 160)

            for (packetIndex, packet) in packets.enumerated() {
                let delay = DispatchTime.now() + .milliseconds(packetIndex * 5)
                DispatchQueue.main.asyncAfter(deadline: delay) {
                    print("Sending stereo PCM file packet \(packetIndex + 1)/\(packets.count)")
                    self.sendAudioPacket(packet)
                }
            }
        } catch {
            print("testSendStereoPCMFile failed:", error.localizedDescription)
        }
    }
}
